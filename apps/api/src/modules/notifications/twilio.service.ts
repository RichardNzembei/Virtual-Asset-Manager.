...
export class TwilioService {
  ...
  // Demo phone numbers that bypass real Twilio
  private readonly demoPhones = ['+254700000001', '+254700000002', '+254700000003', '+254700000004'];
  ...
  async sendOtp(phone: string): Promise<{ success: boolean; status: string }> {
    if (this.demoPhones.includes(phone)) {
      this.logger.log(`[DEMO] OTP sent to ${phone} (demo bypass — use code 123456)`);
  ...
  async verifyOtp(phone: string, code: string): Promise<{ success: boolean; status: string }> {
    if (this.demoPhones.includes(phone)) {
      ...
      this.logger.log(`[DEMO] OTP verify for ${phone}: ${valid ? 'approved' : 'rejected'}`);
